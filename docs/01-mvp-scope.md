...
scope." The PRD's P0 list is a good statement of ambition, but taken literally it
is not an MVP: it contains roughly eighteen months of work, including several
items whose cost is dominated by a single hard problem each (optical flow, HDR,
scopes). What follows is the cut we can actually ship, and — more usefully — the
reasoning for each cut, so the line can be re-litigated when circumstances change.

## The rule used to cut

A feature stays in the MVP if **removing it would make a target persona abandon
the app**, not merely complain about it. Everything else moves to Phase 1.
...
The union of the "abandons over" column is the MVP. That is a much smaller list
than the PRD's P0, and every removal below names which persona was checked.

## In the MVP

### Timeline and editing — the whole thing, no cuts
...
- 6 video + 6 audio tracks (not 12 — see cuts)
...
Speed curves stayed in against the PRD's own P1 placement. The reason is
...
| All server-side AI | Phase 1 | This is the largest single cut. See below. |
...
### On cutting all server-side AI from launch

This is the biggest departure from the PRD, which places premium captions, TTS
and object removal at P0.

The argument for cutting: every server capability adds inference cost per use,
...
defensible because the queue is built — the capabilities can be enabled
